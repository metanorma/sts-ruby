...
-->
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:oasis="-//OASIS//DTD XML Exchange Table Model 19990315//EN" xmlns:xlink="http://www.w3.org/1999/xlink"
    xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:css="http://www.iso.org/ns/css-parser" xmlns:css2cals="http://www.iso.org/ns/css2cals" exclude-result-prefixes="xs xlink css css2cals">
    ...
    <xsl:template match="table" mode="xhtml2cals">
        ...
        <!--<xsl:copy-of select="$expanded-spans-table"></xsl:copy-of>-->
        ...
        <xsl:variable name="num-cols">
            <xsl:choose>
                ...
                <xsl:otherwise>
                    ...
                    <xsl:value-of select="count(./tr[1]/td[not(@colspan)]) + sum(./tr[1]/td/@colspan)"/>
                </xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
        <oasis:table>
    ...
    <xsl:template match="col" mode="convert-to-cals">
        ...
        <oasis:colspec colnum="{$colnum}" colname="col{$colnum}">
        ...
        <xsl:choose>
            ...
            <xsl:when test="count($processed-row/node())=0">
                ...
                <xsl:variable name="first-row" as="element()+">
                    <xsl:element name="tr" inherit-namespaces="no">
                        <xsl:copy-of select="oasis:expand-col-spans($source-rows/tr[1])"/>
                    </xsl:element>
                </xsl:variable>
            ...
            <xsl:otherwise>
                ...
                <xsl:variable name="row" as="element()">
                    <xsl:copy-of select="oasis:expand-table-row($source-rows,$processed-row,$row-count)"/>
                </xsl:variable>
    ...
    <xsl:function name="oasis:expand-table-row" as="element()">
        ...
        <xsl:variable name="next-row">
            <xsl:element name="tr" inherit-namespaces="no">
                <xsl:copy-of select="oasis:expand-row-spans($table-block/tr[$row],$expanding-row)"/>
            </xsl:element>
        </xsl:variable>
        ...
        <xsl:variable name="next-row-expanded" as="element()">
            <xsl:element name="tr" inherit-namespaces="no">
                <xsl:copy-of select="oasis:expand-col-spans($next-row/node())"/>
            </xsl:element>
        </xsl:variable>
    ...
    <xsl:function name="oasis:expand-col-spans" as="item()+">
    ...
    <xsl:function name="oasis:expand-row-spans" as="item()+">
        <xsl:param name="source-rows" as="element()+"/>
        ...
        <xsl:for-each select="$modified-row/*">
            <xsl:choose>
                ...
                <xsl:otherwise>
                    ...
                    <xsl:copy-of select="oasis:select-cell($current-column - $spanned-row-cells,$source-rows,1,0)" copy-namespaces="no"/>
                </xsl:otherwise>
            </xsl:choose>
        </xsl:for-each>
    </xsl:function>
    ...
    <xsl:function name="oasis:select-cell">
        ...
        <xsl:choose>
            ...
            <xsl:when test="$src-column-no &gt; $current-span-col-total+$current-span">
                <xsl:copy-of select="oasis:select-cell($src-column-no,$row,$current-column-count+1, $current-span+ $current-span-col-total)" copy-namespaces="no"/>
            </xsl:when>
            <xsl:otherwise>
    ...
    <xsl:function name="css2cals:showBorderRight" as="xs:boolean">
        ...
        <xsl:choose>
            ...
            <xsl:when
                test="not(($css/css:border-right-style, $css/css:border-right, $css/css:border-style, $css/css:border)[1]/(css:none,css:hidden)) and ($css/css:border-right-width/css:dimension, $css/css:border-right/css:dimension[1], $css/css:border/css:dimension[1])[1] &gt; 0">
                <xsl:copy-of select="true()"/>
            </xsl:when>
            <xsl:otherwise>
                <xsl:copy-of select="false()"/>
    ...
    <xsl:function name="css2cals:showBorderLeft" as="xs:boolean">
        ...
        <xsl:choose>
            <!-- specific settings for border-right overwrite general -->
            <xsl:when
                test="not(($css/css:border-left-style, $css/css:border-left, $css/css:border-style, $css/css:border)[1]/(css:none,css:hidden)) and ($css/css:border-left-width/css:dimension, $css/css:border-left/css:dimension[1], $css/css:border/css:dimension[1])[1] &gt; 0">
                <xsl:copy-of select="true()"/>
            </xsl:when>
            <xsl:otherwise>
                <xsl:copy-of select="false()"/>
    ...
    <xsl:function name="css2cals:showBorderBottom" as="xs:boolean">
        ...
        <xsl:choose>
            ...
            <xsl:when
                test="not(($css/css:border-bottom-style, $css/css:border-bottom, $css/css:border-style, $css/css:border)[1]/(css:none,css:hidden)) and ($css/css:border-bottom-width/css:dimension, $css/css:border-bottom/css:dimension[1], $css/css:border/css:dimension[1])[1] &gt; 0">
                <xsl:copy-of select="true()"/>
            </xsl:when>
            <xsl:otherwise>
                <xsl:copy-of select="false()"/>
    ...
    <xsl:function name="css2cals:showBorderTop" as="xs:boolean">
        ...
        <xsl:choose>
            <!-- specific settings for border-bottom overwrite general -->
            <xsl:when
                test="not(($css/css:border-top-style, $css/css:border-top, $css/css:border-style, $css/css:border)[1]/(css:none,css:hidden)) and ($css/css:border-top-width/css:dimension, $css/css:border-top/css:dimension[1], $css/css:border/css:dimension[1])[1] &gt; 0">
                <xsl:copy-of select="true()"/>
            </xsl:when>
            <xsl:otherwise>
                <xsl:copy-of select="false()"/>
